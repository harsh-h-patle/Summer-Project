...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while flag:
        ...
        for i in range(nS):
            a = policy[i]
            sum = 0
            for k in range(len(P[i][a])):
                sum += P[i][a][k][0] * (P[i][a][k][2] + gamma * p_value_function[P[i][a][k][1]])
            value_function[i] = sum
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for i in range(nS):
        ...
        for j in range(nA):
            sum1 = np.zeros(len(P[i][j]),dtype=float)
            for k in range(len(P[i][j])):
                sum1[k] = P[i][j][k][0]*(P[i][j][k][2] + gamma*value_from_policy[P[i][j][k][1]])
            action_value_function[j] = sum(sum1)
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
     ...
     while flag:
        ...
        for i in range(nS):
            a = policy[i]
            q_value = np.zeros(len(P[i][a]),dtype=float)
            for k in range(len(P[i][a])):
                q_value[k] = P[i][a][k][0] * (P[i][a][k][2] + gamma * pre_value_function[P[i][a][k][1]])
            value_function[i] = sum(q_value)
        ...
        for i in range(nS):
            for j in range(nA):
                sum2 = np.zeros(len(P[i][j]),dtype=float)
                for k in range (len(P[i][j])):
                    sum2[k] = P[i][j][k][0] * (P[i][j][k][2] + gamma * value_function[P[i][j][k][1]])
                action_value_function[j] = sum(sum2)   
...
def render_single(env, policy, max_steps=10000):
    """
